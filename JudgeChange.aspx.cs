...
public partial class judgeChange: System.Web.UI.Page
{
    static string j = "JudgeLow";
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["teaId"] != null)
        {
            GridView1.DataSource = Manage.ShowData(j);
            //GridView1.DataSource = Manage.ShowData("JudgeLow");
            GridView1.DataBind();
        }
        else
        {
   ...
   //删除操作
    ...
    protected void GridView1_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        question = QuestionChangeManager.GetJudgeQuestion(Id,j);  //通过GetUser方法的userid索引，获取用户数据
    ...
    {
        JudgeHigh updateQuestion = new JudgeHigh();
        ...
        int n = QuestionChangeManager.UpdateJudgeQuestion(updateQuestion, j);
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        JudgeHigh newquestion = new JudgeHigh(question, answer, explain,suggestion);
        //调用AddUser方法，添加新用户信息
       int n = QuestionChangeManager.AddJudgeQuestion(newquestion,j);
