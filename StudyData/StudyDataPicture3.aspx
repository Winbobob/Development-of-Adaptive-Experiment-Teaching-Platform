<%@ Page Language="C#" MasterPageFile="~/StudyData/StudyDataMasterPage.master" AutoEventWireup="true" CodeFile="StudyDataPicture3.aspx.cs" Inherits="StudyData_StudyDataPicture3" Title="无标题页" %>
...
<script type="text/javascript">
    ...
    function openWinCenter() {
        ...
        GB_showCenter("知识点3例题（图片型）", "../example/examplePicture3.aspx", 350, 630);
    }
</script> 
...
<asp:Panel ID="PanelAll" runat="server"  ScrollBars="Vertical"  Height="820px">
  ...
  <b>1)构造函数</b><br />
  构造函数和其他函数很像，除了以下两点：<br />
i)类的构造函数名称与类名相同<br />
ii)构造函数不能有返回类型<br />
<asp:Image ID="Image2" runat="server"   ImageUrl="~/DataImage/3.1.jpg"  /><br />
...
{<br />
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 在C#程序中，用户不能显式地调用析构函数，通过静态类GC的collect方法可强制程序进行垃圾回收，从而间接调用相关对象的析构函数。<br />
  <b>总结：比较析构函数和构造函数</b><br />
  <asp:Image ID="Image1" runat="server"   ImageUrl="~/DataImage/3.2.jpg"  />
  </p>
...
{<br />
   ...
   <asp:Image ID="Image3" runat="server"   ImageUrl="~/DataImage/3.3.jpg"  /><br />
   通过对象的属性来访问集合元素的示例代码如下：<br />
   <asp:Image ID="Image4" runat="server"   ImageUrl="~/DataImage/3.4.jpg"  /><br />
   解析：<br />
    <asp:Image ID="Image5" runat="server"   ImageUrl="~/DataImage/3.5.jpg"  /><br />
   索引函数则对属性作了进一步扩展，它能够以数组的方式来访问对象内部的多项数据。那是使用索引函数的classmateList类定义：<br />
    <asp:Image ID="Image6" runat="server"   ImageUrl="~/DataImage/3.6.jpg"  /><br />
    方法二：<br />
...
索引函数的参数类型必须为整数或字符串；此外，它维护的集合对象不一定是数组，也可以是多个离散的字段。<br />
例如在下面的Teacher类包含了homePhone、officePhone、mobilePhone这3个私有字段，并通过索引函数来控制它们的读写：<br />
 <asp:Image ID="Image7" runat="server"   ImageUrl="~/DataImage/3.7.jpg"  />
   </p>
