<%@ Page Language="C#" MasterPageFile="~/StudyData/StudyDataMasterPage.master" AutoEventWireup="true" CodeFile="StudyDataText2.aspx.cs" Inherits="StudyData_StudyDataText2" Title="无标题页" %>
...
<script type="text/javascript">
    ...
    function openWinCenter() {
        ...
        GB_showCenter("知识点2例题（文字型）", "../example/exampleText2.aspx", 350, 630);
    }
</script> 
...
(1)返回类型：表示方法返回的计算结果的类型，如果没有返回类型，则用void<br />
(2)方法名：表示方法的知识<br />
(3)参数列表：包含在方法名的一对括号中，表示方法的参数集合。<br />
(4)执行体：包含在一队大括号中的方法执行代码，为空时不执行任何操作。</p>
...
对数组型参数的使用有着严格的规定：<br />
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      C#应用程序的主方法Main也可以使用数组型参数，但它额外规定数组元素的类型必须是字符串string，此时数组型参数就表示程序所能接受的参数集合。</p>
  <h3 class="style4">2.3方法的重载</h3>
 <p class="style3">C#中允许一个类中包含多个同名的方法，只要它们的参数数量或类型不完全相同，这方法叫做方法的重载。<br />
 class Program<br />
{<br />
     　Public static void Swap(ref int x,ref int y)<br />
     　{<br />
　　Int z=x;<br />
　　X=y;<br />
     　　Y=z;<br />
     　}<br />
     　Public static void Swap(ref double x,ref double y)<br />
     　{<br />
　　double z=x;<br />
　　X=y;<br />
　　Y=z;<br />
     　}<br />
     　public static void Swap(ref string x，ref string y)<br />
     　{<br />
　　String z=x;<br />
　　X=y;<br />
　　Y=z;<br />
...
在使用过程中，程序会根据传递给方法的实参类型来确定具体调用哪一个方法:<br />
Double x=12.5,y=20;<br />
Program.Swap(ref  x,ref  y);	//调用第2种重载形式<br />
String s1=”Apple”,s2=”Orange”;<br />
Program.Swap(ref  x,ref  y);	//调用第3种重载形式<br />
不过，如果两个方法名称和参数列表都相同，而仅仅是返回类型不同，那么C#认为它们不能构成重载。<br />
例如，一个类中不能同时包含下面两个方法:<br />
Public int Add(int y)<br />
      {<br />
      &nbsp;&nbsp;&nbsp; Return 100+y;<br />
      }<br />
      Public double Add(int y )<br />
      {<br />
      &nbsp;&nbsp;&nbsp; Return 1.5*y;<br />
      }<br />
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      此外，由于输出型参数只是在引用型参数的基础上增加了特殊限制，因此只有ref和out型参数的区别也不能构成重载，如二个类中不能同时包含Swap(ref int 
      x, ref int y)和Swap(ref int x,out int y)两个方法。
</p>
</asp:Panel>
